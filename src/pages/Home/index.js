...
const Page = () => {
  ...
  // création d'une const last qui vient chercher la dernière prestation
  const last = data && data.events && data.events.length > 0
  ? [...data.events].sort((evtA, evtB) => 
  new Date(evtB.date) - new Date(evtA.date))[0] : null;
  ...
  return <>
    ...
    <footer className="row">
      <div className="col presta">
        ...
        {/* Ajout de "last &&" avant le 'EventCard' dans le cas ou 'last' ne sra pas défini */}
        {last && (
        <EventCard
